...
const ActivitiesCreation = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchUserDisciplines = async () => {
      try {
        const response = await axios.get(`http://192.168.1.12:8080/discipline/user/${user.id}`);
  ...
  const handleSaveActivity = async () => {
    try {
      ...
      const response = await axios.post('http://192.168.1.12:8080/activities', activityData);
